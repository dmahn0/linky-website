...
CREATE POLICY "Owners can manage their spaces" ON spaces
    FOR ALL USING (auth.uid() = (SELECT uid FROM users WHERE id = owner_id));
...
CREATE POLICY "Business owners can create jobs" ON jobs
    FOR INSERT WITH CHECK (auth.uid() = (SELECT uid FROM users WHERE id = business_id));

CREATE POLICY "Related users can update jobs" ON jobs
    FOR UPDATE USING (
        auth.uid() = (SELECT uid FROM users WHERE id = business_id) OR
        auth.uid() = (SELECT uid FROM users WHERE id = partner_id)
    );
...
CREATE POLICY "Admins can manage education bookings" ON education_bookings
    FOR ALL USING (
        EXISTS (SELECT 1 FROM users WHERE uid = auth.uid() AND type = 'admin')
    );
